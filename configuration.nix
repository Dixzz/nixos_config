...
in
{
  ...
  # If enabling NVIDIA, you will be prompted to configure hardware.nvidia
   hardware.nvidia = {
     ...
     prime = { # For hybrid graphics (laptops), configure PRIME:
       ...
       offload.enable = true; # Or disable PRIME offloading if you don't care
  ...
  users.users.fed = {
    isNormalUser = true;
    #initialPassword = "root"; # SECURITY: Change this password after first login with `passwd`
    extraGroups = [
      "wheel"
      "networkmanager"
      "video"
  ...
  hydenix = {
    ...
    locale = "en_IN"; # REQUIRED: Set locale/language (examples: "en_US.UTF-8", "en_GB.UTF-8", "de_DE.UTF-8")
